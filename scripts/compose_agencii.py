"""
Frame the Agencii hero screenshot as a single premium browser-window "artwork"
on the dark brand canvas (amber + cool glows, faint dot grid, soft drop shadow,
slight tilt) — so the intentionally dark hero reads as deliberate, not washed out.

Source: web/public/projects/agencii-src.png  (landscape hero capture)
Output: web/public/projects/agencii.png

    python web/scripts/compose_agencii.py
"""
...
SRC = os.path.join(SRC_DIR, "agencii-src.png")
OUT = os.path.join(SRC_DIR, "agencii.png")
...
def cover_top(img, w, h):
    """Resize so width matches, crop to h from the top (keeps logo + hero)."""
    img = img.convert("RGB")
    sw, sh = img.size
    scale = w / sw
    nh = int(sh * scale + 0.5)
...
def main():
    ...
    gd.ellipse((W - 1080, -540, W + 360, 720), fill=(255, 182, 0, 72))   # amber, top-right
    gd.ellipse((-440, H - 720, 680, H + 440), fill=(108, 99, 255, 42))   # violet, bottom-left
    ...
    win = browser_window(SRC, "agencii.ai")
